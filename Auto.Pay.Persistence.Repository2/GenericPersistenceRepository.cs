...
namespace Auto.Pay.Persistence.Repository
{
        ...
        where TEntity : class, IEntityPersistence
    {
        ...
        public virtual T FindFirstOrDefault<T>(Expression<Func<T, bool>> predicate) where T : class
        {
            if (predicate != null)
            {
                T entity = _contexto.Set<T>().FirstOrDefault(predicate);

                return entity;
            }
            else
            {
                throw new ArgumentNullException("Predicate value must be passed to FindSingle<T>.");
            }
        }

        public virtual T FindSingle<T>(Expression<Func<T, bool>> predicate) where T : class
        {
            if (predicate != null)
            {
                return _contexto.Set<T>().Single(predicate);
            }
            else
            {
                throw new ArgumentNullException("Predicate value must be passed to FindSingle<T>.");
